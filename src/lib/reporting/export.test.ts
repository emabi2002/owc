...
const firstReference = report.rows[0]?.reference ?? "";
...
describe("OWC management report exports", () => {
  test("CSV carries report metadata, authoritative row count and source rows", () => {
    ...
    expect(csv).toContain(firstReference);
    expect(
      csv
        .split("\n")
        .filter((line) => report.rows.some((row) => line.startsWith(row.reference)))
        .length,
    ).toBe(report.recordCount);
  });
  ...
  test("spreadsheet export is Excel-compatible tabular text with matching totals", () => {
    ...
    expect(text).toContain(firstReference);
    expect(
      text
        .split("\n")
        .filter((line) => report.rows.some((row) => line.startsWith(row.reference)))
