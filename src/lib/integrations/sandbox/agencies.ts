...
export function verifyIdentity(nid: string) {
  ...
  return makeSandboxEnvelope("nid", "verify_identity", matched
    ? {
        matched: true,
...
export function verifyEmployer(registrationNo: string) {
  ...
  return makeSandboxEnvelope("ipa", "verify_company", active
    ? {
        active: true,
...
export function checkTaxCompliance(tin: string) {
  ...
  return makeSandboxEnvelope("irc", "check_compliance", matched
    ? {
        found: true,
...
export function verifyEmployment(employeeNo: string) {
  ...
  return makeSandboxEnvelope("employer", "verify_employment", employed
    ? {
        employed: true,
...
export function verifyMedicalCertificate(certificateNo: string) {
  ...
  return makeSandboxEnvelope("medical", "verify_certificate", valid
    ? {
        valid: true,
...
export function verifyInsurancePolicy(policyNo: string) {
  ...
  return makeSandboxEnvelope("insurance", "verify_policy", active
    ? {
        active: true,
...
export function verifyBankAccount(accountReference: string) {
  ...
  return makeSandboxEnvelope("bank", "verify_account", verified
    ? {
        verified: true,
